...
`backend/src/contracts` と `schemas/` の単一ソース化が未完である。

現状は frontend 側の参照を `schemas/` 経由へ寄せているが、backend 側に契約定義が残っている。  
Full Completion では、契約の正をどこに置くかを明確にし、frontend / backend の参照経路を揃える必要がある。
