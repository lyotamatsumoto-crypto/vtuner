...
### Phase 16-2 close status（AI / JSON Studio validation 接続 最小完了）

- AI / JSON Studio に `返答テンプレートJSON` target を追加済み
- `validateReplyTemplatesJson()` を UI validation に接続済み
- JSON parse error と schema validation error を分離して表示する
- valid 時は parsed `category count` / `category names` を表示する
- invalid 時は validator errors を表示する
- valid / invalid sample を挿入できる
- `返答テンプレートJSON` では Import Queue 登録を無効化している
- persona validation / persona Import Queue 登録は維持している
- backend / compile / Overlay / PreviewTest は未変更
- replyTemplates の Queue 登録、採用、compile、runtime反映は後続で扱う

### Phase 16-3 close status（replyTemplates Import Queue 前段接続 最小完了）

- `generation_target` に `reply_templates` を追加済み
- 既存 AI JSON Import Queue を最小拡張した
- AI / JSON Studio から validation OK の `返答テンプレートJSON` を Import Queue に登録できる
- validation NG / parse error / schema validation error の場合は登録しない
- Queue 一覧で `人格JSON` / `返答テンプレートJSON` を区別できる
- replyTemplates item はまだ Adopted Changes へ採用できない
- UI 側と App 側の二重ガードで replyTemplates item の採用を防いでいる
- persona の validation / queue登録 / 採用導線は維持している
