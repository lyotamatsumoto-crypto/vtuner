...
13. Phase 12+: Extension Phases（詳細は `ROADMAP_TO_EXTENSION.md`）
...
### Purpose
条件付きMVP完了後の拡張を、5画面責務分離を維持したまま段階導入する。

### Scope
- 拡張フェーズの順序と優先度管理
- Character Profile 保存、未保存状態表示、読み上げのみモード、発言対象などの導入

### Notes
- 本章は進行上の接続点のみを持つ
- 各拡張の詳細は `ROADMAP_TO_EXTENSION.md` を正とする
