...
- Current phase: Phase 12+ Post-MVP Roadmaps / Completion Roadmap C1-3
- Current status: Conditional MVP Complete 後の Completion Roadmap C1 を実施。Review / Compile 系 contract は `schemas/reviewCompile/*` を SSOT とし、`schemas` 独立 build unit と backend build output 整理は完了
- Next phase: Completion Roadmap C2（AI JSON Import Queue and Validation Flow）または Extension Roadmap の次の最小項目
- Last completed phase: Completion Roadmap C1: Contract SSOT
...
13. Phase 12+: Post-MVP Roadmaps（Completion は `ROADMAP_TO_COMPLETION.md`、Extension は `ROADMAP_TO_EXTENSION.md`）
...
### Purpose
条件付きMVP完了後の後続計画を、Completion と Extension に分離して段階導入する。

### Scope
- Completion 側（基盤強化）と Extension 側（機能拡張）の参照先管理
- Extension 側では Character Profile 保存、未保存状態表示、読み上げのみモード、発言対象などを段階導入

### Notes
- 本章は進行上の接続点のみを持つ
- Completion の詳細は `ROADMAP_TO_COMPLETION.md` を正とする
- Extension の詳細は `ROADMAP_TO_EXTENSION.md` を正とする
- Completion Roadmap C1 では、Review / Compile 系 contract の正を `schemas/reviewCompile/*` に寄せ、backend は `schemas/dist` を参照する façade 構成へ整理済み
- C1 後の backend build output は backend 本体中心であり、`backend/dist/schemas` を生成しない
